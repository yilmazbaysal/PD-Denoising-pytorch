...
class CBDNet(nn.Module):
    def __init__(self, channels=1):
        ...
        self.fcn = FCN(channels)
        self.unet = UNet(channels)

    def forward(self, x, external_noise_level=None):
        noise_level = self.fcn(x)

        if external_noise_level is not None:
            concat_img = torch.cat([x, external_noise_level], dim=1)
        else:
