...
from models.dncnn import DnCNN_c, Estimation_direct
from utils import *
...
def main():
    ...
    device_ids = [0]
    model = nn.DataParallel(CBDNet(channels=1), device_ids=device_ids)
    model.eval()
    model_info = torch.load('../../logs/cbdnet_all.pth.tar', map_location=torch.device('cpu'))
